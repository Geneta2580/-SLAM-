...
void computeAngle(const cv::Mat &image, vector<cv::KeyPoint> &keypoints) {
	...
	// START YOUR CODE HERE (~7 lines)
        ...
        for(int dx = -half_patch_size;dx < half_patch_size;++dx)
        {
            ...
            {
               m01 += dy * image.at<uchar>(kp.pt.x + dx, kp.pt.y + dy);
               m10 += dx * image.at<uchar>(kp.pt.x + dx, kp.pt.y + dy);
            }
        }
    // END YOUR CODE HERE
...
void computeORBDesc(const cv::Mat &image, vector<cv::KeyPoint> &keypoints, vector<DescType> &desc) {
    ...
    for (auto &kp: keypoints) {
        ...
        for (int i = 0; i < 256; i++) {
            // START YOUR CODE HERE (~7 lines)
            double cos_angle = std::cos(kp.angle * pi / 180);
            double sin_angle = std::sin(kp.angle * pi / 180);

            cv::Point2f p(ORB_pattern[4 * i], ORB_pattern[4 * i + 1]); // 以一定的pattern,以关键点为中心进行旋转,所以这里取得是相对坐标
            cv::Point2f q(ORB_pattern[4 * i + 2], ORB_pattern[4 * i + 3]);

            cv::Point2f pp(cos_angle * p.x - sin_angle * p.y, sin_angle * p.x + cos_angle * p.y) ; // 以关键点为中心进行旋转
            cv::Point2f qq(cos_angle * q.x - sin_angle * q.y, sin_angle * q.x + cos_angle * q.y) ;
            pp = pp + kp.pt;
            qq = qq + kp.pt;

            if(qq.x >= 0 && qq.x < image.cols && qq.y >= 0 && qq.y < image.rows && pp.x >= 0 && pp.x < image.cols && pp.y >= 0 && pp.y < image.rows) // 边界判断
            {
                if(image.at<uchar>(pp.y, pp.x) > image.at<uchar>(qq.y, qq.x)){d[i]=false;}
                else{d[i]=true;}
                flag = 1;
            }
            else{flag = 0; break;}
        ...
        if(flag == 0){desc.push_back({});}
        else{desc.push_back(d);}
    }
...
void bfMatch(const vector<DescType> &desc1, const vector<DescType> &desc2, vector<cv::DMatch> &matches) {
    ...
    for(int i = 0; i < desc1.size(); i++)
    {
        ...
        cv::DMatch m{i, 0, 256};  // queryIdx trainIdx distance
        for(int j = 0; j < desc2.size(); j++)
        {
            ...
            for(int k = 0; k < 256; k++)
            {
                distance += desc1[i][k]^desc2[j][k];       // 计算汉明距离
            }
            // cout << distance << endl;
            if(distance < d_max && distance < m.distance)  // 排序,一直赋最小值
            {
                m.distance = distance;
                m.trainIdx = j;
            }
